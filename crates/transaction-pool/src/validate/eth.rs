    ...
    Evm: ConfigureEvm,
{
    ...
    pub fn validate_sender_bytecode(
        ...
        // transactions.
        if let Some(code_hash) = sender.bytecode_hash.as_ref().filter(|hash| **hash != KECCAK_EMPTY)
        {
...
mod tests {
    ...
    #[test]
    fn accepts_sender_with_empty_bytecode_hash() {
        let transaction = get_transaction();
        let provider = MockEthProvider::default().with_genesis_block();
        provider.add_account(
            transaction.sender(),
        ...
        assert!(validator.validate_one(TransactionOrigin::External, transaction).is_valid());
    }
