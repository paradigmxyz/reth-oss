    ...
    Evm: ConfigureEvm<Primitives: NodePrimitives<Block = Provider::Block>> + 'static,
{
    fn generate_witness(
        ...
        tracing::info!(target: "engine_ssz_proxy", %block_hash, "generating execution witness");
    ...
    Api: EngineSszApi,
{
    ...
    match endpoint {
        ...
        EngineSszEndpoint::PayloadBodiesByHash => {
            ...
            let hashes: Vec<B256> =
                match request.into_body().collect().await.map(|body| body.to_bytes()) {
            ...
            let status = match EngineSszPayloadStatus::try_from(status) {
                ...
                Err(err) => {
                    tracing::info!(
                        ...
                        version,
                        %err,
                        "failed to convert newPayload response to SSZ type"
                    );
    ...
    ChainSpec: EthereumHardforks + Send + Sync + 'static,
{
    ...
    match response {
        Ok(status) => Ok(status),
        Err(err) => Err(text_response(STATUS_INTERNAL_SERVER_ERROR, err.to_string())),
    }
}
            ...
            let updated = match ForkchoiceUpdateResponse::try_from(updated) {
                ...
                Err(err) => {
                    tracing::info!(
                        ...
                        version,
                        %err,
                        "failed to convert forkchoice response to SSZ type"
                    );
                    return text_response(STATUS_INTERNAL_SERVER_ERROR, err.to_string())
                }
            };
