...
pub struct Options {
    ...
    pub color: bool,
    /// Emit collapsible `GitHub` Actions groups for each mismatch.
    pub github_groups: bool,
}
...
pub async fn run(manifest_path: &Path, options: Options) -> Result<()> {
    ...
    for method in manifest.methods.into_iter().filter(|method| {
        ...
        if detail.is_some() {
            mismatches += 1;
            let red = if options.color { "\x1b[31m" } else { "" };
            let reset = if options.color { "\x1b[0m" } else { "" };
            if options.github_groups {
                println!("::group::FAIL {} ({})", method.id, method.method);
            } else {
                println!("{red}FAIL{reset} {} ({})", method.id, method.method);
            }
            ...
            passed += 1;
            let green = if options.color { "\x1b[32m" } else { "" };
            let reset = if options.color { "\x1b[0m" } else { "" };
            println!("{green}PASS{reset} {} ({})", method.id, method.method);
        }
    ...
    println!(
        "Geth/Reth RPC compatibility: {selected_count} selected, {passed} passed, {mismatches} mismatches ({:.2}s)",
