...
/// Returns the timestamp increment to use for filler/auto-generated simulated blocks on `chain`.
///
/// Uses [`Chain::average_blocktime_hint`] when available, falling back to
/// [`SIMULATE_FALLBACK_TIMESTAMP_INCREMENT`] otherwise.
fn chain_timestamp_increment(chain: Chain) -> u64 {
    chain
...
///   The block time defaults to [`Chain::average_blocktime_hint`] for `chain`, falling back to
///   [`SIMULATE_FALLBACK_TIMESTAMP_INCREMENT`] when no hint is registered;
...
where
    H: BlockHeader,
{
    let timestamp_increment = chain_timestamp_increment(chain);
...
mod tests {
    ...
    fn sanitize_chain_fills_gaps_with_empty_blocks() {
        ...
        let out = sanitize_chain(blocks, &parent, Chain::mainnet(), 256).unwrap();
        assert_eq!(out.len(), 3);
    ...
    fn sanitize_chain_uses_chain_blocktime_hint() {
        ...
        let out = sanitize_chain(blocks, &parent, Chain::optimism_mainnet(), 256).unwrap();
        let times: Vec<u64> =
            out.iter().map(|b| b.block_overrides.as_ref().unwrap().time.unwrap()).collect();
    ...
    fn sanitize_chain_falls_back_when_chain_has_no_hint() {
        ...
        let out = sanitize_chain(blocks, &parent, Chain::from_id(123_456_789), 256).unwrap();
        let times: Vec<u64> =
            out.iter().map(|b| b.block_overrides.as_ref().unwrap().time.unwrap()).collect();
    ...
    fn sanitize_chain_rejects_non_increasing_number() {
        ...
        let err = sanitize_chain(vec![block_with_number(10)], &parent, Chain::mainnet(), 256)
            .unwrap_err();
    ...
    fn sanitize_chain_enforces_max_blocks() {
        let parent = parent_at(0, 0);
        let err = sanitize_chain(vec![block_with_number(257)], &parent, Chain::mainnet(), 256)
            .unwrap_err();
        assert!(matches!(err, EthApiError::Other(_)));
    }
}
